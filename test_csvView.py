...
class MyWindow(QtWidgets.QWidget):
    def __init__(self, fileName, parent=None):
        ...
        self.filebtn = QtWidgets.QPushButton("Load Model")
        self.filebtn.clicked.connect(self.openmodel)
        self.predictbtn = QtWidgets.QPushButton("Predict")
        self.predictbtn.clicked.connect(self.predictData)
        ...
        grid.addWidget(self.filebtn,2,0,1,1)
        grid.addWidget(self.predictbtn,3,0,1,1)
        grid.addWidget(self.browser,2,1,2,8)
        self.setLayout(grid)
    ...
    def predictData(self):
        ...
        df = pd.read_csv(self.dfname, sep= '\t')
        X_test = df.values
        loaded_model = pickle.load(open(self.dfname,"rb"))
        result = loaded_model.predict(X_test)
        self.browser.setText(loaded_model.show_models)

    def openmodel(self):
        what = QtWidgets.QFileDialog.getOpenFileName(self)
        self.browser.setText(what[0])
        self.browser.append(self.dfname)
        self.browser.append("Accuracy rate : 80.69390033783475")
